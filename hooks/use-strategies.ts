      ...
      try {
        ...
        const transformedStrategies: Strategy[] = data.map((strategy: any) => {
          ...
          return {
            ...
            summary: `${parsedRules.length} alerts • ${strategy.timeframe === 0 ? 'any timeframe' : strategy.timeframe + 'm'} • ${strategy.threshold > 0 ? 'BUY' : 'SELL'} threshold`,
  ...
  const createStrategy = async (strategyData: {
    ...
    try {
      ...
      const transformedStrategy: Strategy = {
        ...
        summary: `${parsedRules.length} alerts • ${newStrategy.timeframe === 0 ? 'any timeframe' : newStrategy.timeframe + 'm'} • ${newStrategy.threshold > 0 ? 'BUY' : 'SELL'} threshold`,
  ...
  const updateStrategy = async (id: string, updates: Partial<{
    ...
    try {
      ...
      const transformedStrategy: Strategy = {
        ...
        rule_groups: parsedRuleGroups, // Database field
        ruleGroups: parsedRuleGroups, // Frontend alias
        summary: `${parsedRules.length} alerts • ${updatedStrategy.timeframe === 0 ? 'any timeframe' : updatedStrategy.timeframe + 'm'} • ${updatedStrategy.threshold > 0 ? 'BUY' : 'SELL'} threshold`,
